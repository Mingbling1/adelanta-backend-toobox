...
class ProveedorRepository(BaseRepository):
    def __init__(self, db: DB_ADMINISTRATIVO) -> None:
        super().__init__(ProveedorModel, db)
    ...
    async def get_all_and_search(
        ...
        filters = []
        if nombre_proveedor:
            search_term = f"%{nombre_proveedor}%"
            filters.append(
                or_(
                    self.entity_class.nombre_proveedor.ilike(search_term),
                    self.entity_class.tipo_proveedor.ilike(search_term),
                    self.entity_class.numero_documento.ilike(search_term),
                )
            )
        if tipo_proveedor:
            filters.append(
                self.entity_class.tipo_proveedor.ilike(f"%{tipo_proveedor}%")
            )
        if numero_documento:
            filters.append(
                self.entity_class.numero_documento.ilike(f"%{numero_documento}%")
            )
